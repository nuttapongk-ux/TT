// pages.js — All page renderers (fully async for Firebase) — no ES modules
"use strict";
...
/* ═══════════════════════════════════════════════════════
   DASHBOARD
═══════════════════════════════════════════════════════ */
PAGES.dashboard = async function (ct) {
  const [s, tickets, cats, agents] = await Promise.all([
    DB.getStats(), DB.getTickets(), DB.getCategories(), DB.getAgents()
  ]);
  ...
  <div class="stats-grid">
    <div class="card">
      <div class="card-header">
        <div class="card-title">🎫 Tickets ล่าสุด</div>
  ...
  ${s.critical > 0 ? `<div class="card" style="border-color:rgba(239,68,68,.3);margin-bottom:20px">
    ...
    <div class="card-body" id="critical-list">
      <div style="color:var(--text-muted);font-size:.85rem">กำลังโหลด...</div>
    </div>
  </div>`: ''}
  ...
  // Render critical cards with SLA (async each)
  if (s.critical > 0) {
    const critList = document.getElementById('critical-list');
    if (critList) {
      const crits = tickets.filter(t => t.priority === 'critical' && t.status !== 'closed');
      const items = await Promise.all(crits.map(async t => {
        const sla = await C.getSLAStatus(t);
        return `<div onclick="navigate('#tickets/${t.id}')" style="display:flex;align-items:center;gap:12px;padding:12px;background:rgba(239,68,68,.07);border:1px solid rgba(239,68,68,.2);border-radius:var(--radius);cursor:pointer;margin-bottom:8px">
...
/* ═══════════════════════════════════════════════════════
   TICKET LIST
═══════════════════════════════════════════════════════ */
let TL = { page: 1, filters: { search: '', status: '', priority: '', category: '' }, sortF: 'createdAt', sortD: 'desc', sel: new Set(), allTickets: [], cats: [] };

PAGES.tickets = async function (ct) {
  TL.page = 1; TL.sel = new Set();
  // Load data
  const [tickets, cats] = await Promise.all([DB.getTickets(), DB.getCategories()]);
  TL.allTickets = tickets; TL.cats = cats;
  ...
  <div class="section-header">
    ...
    <div style="display:flex;gap:10px">
      ...
      ${DB.can('create') ? `<button class="btn btn-accent btn-sm" onclick="navigate('#tickets/new')">➕ สร้าง Ticket</button>` : ''}
    </div>
  </div>
  ...
  document.getElementById('tl-all').addEventListener('change', e => { PAGES._getFiltered().forEach(t => e.target.checked ? TL.sel.add(t.id) : TL.sel.delete(t.id)); PAGES._renderTable(); });
  PAGES._renderTable();
};

PAGES._getFiltered = function () {
  let ts = [...TL.allTickets];
...
PAGES._renderTable = function () {
  ...
  const catMap = Object.fromEntries(TL.cats.map(c => [c.id, c]));
  const tbody = document.getElementById('tl-tbody'); const pgEl = document.getElementById('tl-page'); if (!tbody) return;
    ...
    tbody.innerHTML = slice.map(t => {
      ...
      return `<tr>
        ...
        <td><div style="display:flex;gap:5px">
          ...
          ${DB.can('editAll') ? `<button class="btn btn-icon btn-outline btn-sm" onclick="navigate('#tickets/edit/${t.id}')">✏️</button>` : ''}
        </div></td>
      </tr>`;
    }).join('');
...
PAGES._bulkClose = async function () {
  showConfirm('ปิด Tickets', 'ต้องการปิด ' + TL.sel.size + ' ticket?', async () => {
    await Promise.all([...TL.sel].map(id => DB.updateTicket(id, { status: 'closed' })));
    TL.allTickets = await DB.getTickets(); TL.sel.clear();
    showToast('success', 'ปิดสำเร็จ', ''); PAGES._renderTable();
  });
};
PAGES._bulkDelete = async function () {
  showConfirm('ลบ Tickets', 'ต้องการลบ ' + TL.sel.size + ' ticket?', async () => {
    await Promise.all([...TL.sel].map(id => DB.deleteTicket(id)));
    TL.allTickets = await DB.getTickets(); TL.sel.clear();
    showToast('success', 'ลบแล้ว', ''); PAGES._renderTable();
  });
};
PAGES._exportCSV = function () {
  const catMap = Object.fromEntries(TL.cats.map(c => [c.id, c.name]));
  const rows = PAGES._getFiltered().map(t => [t.id, `"${(t.title || '').replace(/"/g, '""')}"`, C.STATUS_LABELS[t.status], C.PRIORITY_LABELS[t.priority], catMap[t.category] || '', t.reporterName, t.reporterEmail || '', t.merchantId || '', t.createdByTeam || '', new Date(t.createdAt).toLocaleString('th-TH')]);
  const csv = [['Ticket ID', 'Title', 'Status', 'Priority', 'Category', 'Reporter', 'Email', 'Merchant', 'Team', 'Created'], ...rows].map(r => r.join(',')).join('\n');
...
═══════════════════════════════════════════════════════ */
PAGES.form = async function (ct, editId) {
  const [ticket, cats] = await Promise.all([
    editId ? DB.getTicketById(editId) : Promise.resolve(null),
    DB.getCategories()
  ]);
  const isEdit = !!ticket;
  ...
  <div class="card-body"><form id="tf-form"><div style="display:flex;flex-direction:column;gap:20px">
    ...
    <div>
      ...
      <div class="form-grid">
        <div class="form-group"><label class="form-label">Merchant ID</label><input class="form-control" id="tf-mid" placeholder="MER-XXXX" value="${C.esc(ticket?.merchantId)}"></div>
        <div class="form-group"><label class="form-label">Transaction Ref</label><input class="form-control" id="tf-txr" placeholder="TXN-YYYYMMDD-XXX" value="${C.esc(ticket?.transactionRef)}"></div>
      </div>
    </div>
    <div>
      ...
      <div class="form-grid cols-3">
        <div class="form-group"><label class="form-label">ชื่อ-นามสกุล <span class="required">*</span></label><input class="form-control" id="tf-name" value="${C.esc(ticket?.reporterName)}"><span class="form-error" id="err-name"></span></div>
        <div class="form-group"><label class="form-label">อีเมล <span class="required">*</span></label><input class="form-control" type="email" id="tf-email" value="${C.esc(ticket?.reporterEmail)}"><span class="form-error" id="err-email"></span></div>
        <div class="form-group"><label class="form-label">เบอร์โทร</label><input class="form-control" id="tf-phone" value="${C.esc(ticket?.reporterPhone)}"></div>
      </div>
    </div>
  </div>
...
PAGES._submitForm = async function (isEdit, editId) {
  ...
  try {
    if (isEdit) { await DB.updateTicket(editId, data); showToast('success', 'บันทึกสำเร็จ', 'อัปเดตแล้ว'); navigate('#tickets/' + editId); }
    else { const t = await DB.createTicket(data); showToast('success', 'สร้าง Ticket สำเร็จ', t.id); navigate('#tickets/' + t.id); }
  } catch (e) { showToast('error', 'เกิดข้อผิดพลาด', e.message); btn.disabled = false; btn.textContent = isEdit ? '💾 บันทึก' : '➕ สร้าง Ticket'; }
};

/* ═══════════════════════════════════════════════════════
   TICKET DETAIL
═══════════════════════════════════════════════════════ */
PAGES.detail = async function (ct, id) {
  ...
  const [t, cats, agents, cmts] = await Promise.all([
    DB.getTicketById(id), DB.getCategories(), DB.getAgents(), DB.getComments(id)
  ]);
  ...
  <div class="section-header" style="margin-bottom:18px">
    ...
    <div style="display:flex;gap:10px">
      ${canEdit ? `<button class="btn btn-outline btn-sm" onclick="navigate('#tickets/edit/${id}')">✏️ แก้ไข</button>` : ''}
      ${DB.can('admin') ? `<button class="btn btn-danger btn-sm" onclick="PAGES._deleteTicket('${id}')">🗑 ลบ</button>` : ''}
    </div>
  </div>
      ...
      <div class="card"><div class="card-header"><div class="card-title">💬 กิจกรรมและความคิดเห็น</div></div>
        <div class="card-body">
          ...
          <div class="comment-input-wrap" style="padding-top:14px;border-top:1px solid var(--border)">
            <div class="user-avatar-sm">${DB.getSession()?.avatar || '??'}</div>
    ...
    <div class="detail-sidebar">
      ...
      <div class="card"><div class="card-header"><div class="card-title">ℹ️ ข้อมูล</div></div>
        <div class="card-body" style="display:flex;flex-direction:column;gap:10px">
          ...
          ${irow('ทีมผู้สร้าง', t.createdByTeam ? `<span class="team-badge" style="background:${TEAMS[t.createdByTeam]?.bg || '#333'};color:${TEAMS[t.createdByTeam]?.color || '#fff'}">${TEAMS[t.createdByTeam]?.icon || ''} ${t.createdByTeam}</span>` : '-')}
  ...
  if (canEdit) {
    document.getElementById('td-save').addEventListener('click', async () => {
      ...
      await DB.updateTicket(id, { status: ns, assignee: na });
      if (ns !== old) {
        const agName = na ? (agents.find(a => a.id === na)?.name || sess?.name || 'Admin') : (sess?.name || 'Admin');
        await DB.addComment(id, { text: `เปลี่ยนสถานะจาก <strong>${C.STATUS_LABELS[old]}</strong> → <strong>${C.STATUS_LABELS[ns]}</strong>`, author: agName, avatar: agName.slice(0, 2).toUpperCase(), type: 'status' });
      }
      showToast('success', 'บันทึกสำเร็จ', `สถานะ: ${C.STATUS_LABELS[ns]}`);
      await PAGES.detail(ct, id);
    });
  }

  const sess = DB.getSession();
  const cinput = document.getElementById('td-cin');
  document.getElementById('td-csend').addEventListener('click', async () => {
    const txt = cinput.value.trim(); if (!txt) { showToast('warning', 'กรุณากรอกความคิดเห็น', ''); return; }
    await DB.addComment(id, { text: txt, author: sess?.name || 'Unknown', avatar: sess?.avatar || '??', type: 'comment' });
...
PAGES._deleteTicket = async function (id) {
  showConfirm('ลบ Ticket', 'ต้องการลบ Ticket นี้อย่างถาวรใช่ไหม?', async () => {
    await DB.deleteTicket(id); showToast('success', 'ลบแล้ว', ''); navigate('#tickets');
  });
};
...
/* ═══════════════════════════════════════════════════════
   ADMIN
═══════════════════════════════════════════════════════ */
PAGES.admin = async function (ct) {
  const [agents, cats, s] = await Promise.all([DB.getAgents(), DB.getCategories(), DB.getSettings()]);
  ...
  <div class="admin-grid">
    ...
    <div class="card">
      <div class="card-header"><div class="card-title">☁️ Firebase Info</div></div>
      <div class="card-body" style="display:flex;flex-direction:column;gap:10px;font-size:.85rem">
        <div style="display:flex;justify-content:space-between"><span style="color:var(--text-muted)">Project</span><span style="color:var(--accent-light)">gloy-8831a</span></div>
        <div style="display:flex;justify-content:space-between"><span style="color:var(--text-muted)">Database</span><span style="color:var(--green);font-size:.75rem">🟢 Connected</span></div>
        <div style="display:flex;justify-content:space-between"><span style="color:var(--text-muted)">Region</span><span>asia-southeast1</span></div>
        <div class="divider"></div>
        <button class="btn btn-danger btn-sm" id="admin-reset">🗑 รีเซ็ตข้อมูล Firebase</button>
        <div style="font-size:.72rem;color:var(--text-muted)">จะลบข้อมูลทั้งหมดจาก Firebase และโหลดข้อมูลตัวอย่างใหม่</div>
      </div>
    </div>
  </div>`;

  document.getElementById('admin-save-sla').addEventListener('click', async () => {
    const sla = {};
    ['critical', 'high', 'medium', 'low'].forEach(p => sla[p] = parseInt(document.getElementById('sla-' + p).value) || 24);
    await DB.saveSettings({ ...s, sla }); showToast('success', 'บันทึก SLA แล้ว', '');
  });
  document.getElementById('admin-add-cat').addEventListener('click', async () => {
    const icon = document.getElementById('nc-icon').value.trim() || '📋';
    const name = document.getElementById('nc-name').value.trim();
    if (!name) { showToast('warning', 'กรุณากรอกชื่อ', ''); return; }
    const cats2 = await DB.getCategories();
    const nc = { id: 'c-' + Date.now(), name, icon, color: '#6b7280' };
    cats2.push(nc); await DB.saveCategories(cats2);
  ...
  document.getElementById('admin-reset').addEventListener('click', () => {
    showConfirm('รีเซ็ตข้อมูล Firebase', 'จะลบทุกอย่างจาก Firebase แล้วโหลดข้อมูลตัวอย่างใหม่ ยืนยัน?', async () => {
      const btn = document.getElementById('admin-reset'); if (btn) { btn.disabled = true; btn.textContent = 'กำลังรีเซ็ต...'; }
      await DB.resetAll();
      showToast('success', 'รีเซ็ต Firebase สำเร็จ', '');
...
PAGES._addAgentModal = function () {
  ...
  ov.querySelector('#ma-ok').onclick = async () => {
    ...
    const ags = await DB.getAgents(); ags.push(ag); await DB.saveAgents(ags);
    document.getElementById('admin-agents').insertAdjacentHTML('beforeend', agCard(ag));
    ov.remove(); showToast('success', 'เพิ่ม Agent', '' + name);
  };
};

window.adminRemoveAgent = async function (id) {
  showConfirm('ลบ Agent', 'ยืนยัน?', async () => {
    const ags = await DB.getAgents();
    await DB.saveAgents(ags.filter(a => a.id !== id));
    document.getElementById('ag-' + id)?.remove(); showToast('success', 'ลบแล้ว', '');
  });
};
window.adminRemoveCat = async function (id) {
  const cats = await DB.getCategories();
  await DB.saveCategories(cats.filter(c => c.id !== id));
  document.getElementById('cc-' + id)?.remove(); showToast('success', 'ลบหมวดหมู่', '');
};
